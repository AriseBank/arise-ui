...
angular.module('ariseApp').controller('appController', ['dappsService', '$scope', '$rootScope', '$http', "userService", "$interval", "$timeout", 'viewFactory', '$state', 'blockService', 'sendTransactionModal', 'registrationDelegateModal', 'serverSocket', 'delegateService', '$window', 'forgingModal', 'errorModal', 'userInfo', 'transactionsService', 'secondPassphraseModal', 'focusFactory', 'gettextCatalog', function (dappsService, $rootScope, $scope, $http, userService, $interval, $timeout, viewFactory, $state, blockService, sendTransactionModal, registrationDelegateModal, serverSocket, delegateService, $window, forgingModal, errorModal, userInfo, transactionsService, secondPassphraseModal, focusFactory, gettextCatalog) {
    ...
    $scope.getPriceTicker = function () {
        $http.get("https://explorer.arise.io/api/getPriceTicker")
            .then(function (response) {
                $scope.btc_usd = Math.floor(response.data.tickers.BTC.USD * 1000000) / 1000000;
                $scope.arise_btc = Math.floor(response.data.tickers.ACO.BTC * 1000000)  / 1000000;
                $scope.arise_usd = Math.floor(response.data.tickers.ACO.USD * 1000000) / 1000000;
            });
    };
    ...
    $scope.getVersion = function () {
        $http.get("/api/peers/version").then(function (response) {
            if (response.data.success) {
                ...
                $http.get("https://login.arise.io/api/peers/version").then(function (response) {
    ...
    $scope.convertToUSD = function (arise) {
        return (arise / 100000000) * $scope.arise_usd;
    };
