...
angular.module('ariseApp').controller('multisignatureModalController', ["$scope", "$http", "multisignatureModal", "viewFactory", "userService", "feeService", "gettextCatalog", function ($scope, $http, multisignatureModal, viewFactory, userService, feeService, gettextCatalog) {
    ...
    $scope.addMember = function (member) {
        ...
        if ($scope.member.trim() == '') {
            ...
            if (buffer.length == 32) {
                var arise = require('arise-js');
                var address = arise.crypto.getAddress($scope.member);
