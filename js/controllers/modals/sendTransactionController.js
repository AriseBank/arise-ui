...
angular.module('ariseApp').controller('sendTransactionController', ['$scope', 'sendTransactionModal', '$http', 'userService', 'feeService', '$timeout', '$filter', function ($scope, sendTransactionModal, $http, userService, feeService, $timeout, $filter) {
    ...
    $scope.isCorrectValue = function (currency, throwError) {
        ...
        if (amount == '') {
            return error('ACO amount can not be blank');
        }
        ...
        if (parts.length == 1) {
            ...
            if (parts[1].length > 8) {
                return error('ACO amount must not have more than 8 decimal places');
            ...
            return error('ACO amount must have only one decimal point');
        }
        ...
        if (amount == '0' && fraction == '00000000') {
            return error('ACO amount can not be zero');
        }
        ...
        if (!/^\d+$/.test(result)) {
            return error('ACO amount contains non-numeric characters');
        }
    ...
    $scope.convertARISE = function (currency) {
    ...
    $scope.sendTransaction = function (secretPhrase, withSecond) {
        ...
        var data = {
            secret: secretPhrase,
            amount: $scope.convertARISE($scope.amount),
